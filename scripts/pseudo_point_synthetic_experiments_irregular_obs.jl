...
println("pseudo_point_synthetic_experiments_irregular_obs")
...
const exp_dir_name = "pseudo_point_scaling_irregular_obs"
...
let
    ...
    let
        ...
        foreach(
            save_dir -> begin
                savefig(plt_lml, joinpath(save_dir, "irregular_lml_plot.tikz"))
                savefig(plt_comp_time, joinpath(save_dir, "irregular_timing_plot.tikz"))
                savefig(
                    plot(plt_lml, plt_comp_time; layout=(2, 1)),
                    joinpath(save_dir, "irregular_lml_timing_plot.tikz"),
                )
            end,
            save_dirs,
        )
