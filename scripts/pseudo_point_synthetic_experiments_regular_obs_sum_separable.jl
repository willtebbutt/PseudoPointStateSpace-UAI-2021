...
println("pseudo_point_synthetic_experiments_regular_obs_sum_separable")
...
const exp_dir_name = "pseudo_point_scaling_regular_obs_sum_separable"
...
let
    ...
    let
        ...
        foreach(
            save_dir -> begin
                savefig(plt_lml, joinpath(save_dir, "regular_lml_plot_sum_separable.tikz"))
                savefig(
                    plt_comp_time,
                    joinpath(save_dir, "regular_timing_plot_sum_separable.tikz"),
                )
                savefig(
                    plot(plt_lml, plt_comp_time; layout=(2, 1)),
                    joinpath(save_dir, "regular_lml_timing_plot_sum_separable.tikz"),
                )
            end,
            save_dirs,
        )
